...
class DeviceSettingsController(AbstractController):
    ...
    def _setup_mvc_connections(self):
        ...
        self._view.device_selection_changed.connect(self._handle_selection_changed)
    ...
    def set_data(self, widget: str, index: typing.Union[QModelIndex, int], data: typing.Any):
        ...
        """
        try:
            if widget == "all_devices" and hasattr(self._model, 'all_devices_model'):
                ...
                # Set data in model
                success = self._model.all_devices_model.setData(model_index, data)
                if success:
                    self._update_ui_state()
                ...
                if isinstance(index, int):
                    model_index = self._model.selected_devices_model.index(index, 0)
                else:
                    model_index = index

                success = self._model.selected_devices_model.setData(model_index, data)
                if success:
                    self._update_ui_state()
                    self.gui_event.emit("data_updated", {"widget": widget, "index": index, "data": data})
    ...
    def _handle_add_device(self, device_data: dict):
        ...
        try:
            ...
            if not self._validate_device_data(device_data):
                self.gui_event.emit("validation_error", {"message": "Invalid device data provided"})
                return

            # Check for duplicate device names
            if self._device_name_exists(device_data.get('name', '')):
                self.gui_event.emit("validation_error", {"message": f"Device '{device_data['name']}' already exists"})
                return
            ...
            if success:
                ...
                self.gui_event.emit("error", {"message": "Failed to add device"})

        except Exception as e:
            self.gui_event.emit("error", {"message": f"Error adding device: {str(e)}"})
    ...
    def _handle_remove_device(self, device_name: str):
        ...
        try:
            ...
            if success:
                ...
                # Emit success event
                self.gui_event.emit("device_removed", {"device_name": device_name})
            else:
                self.gui_event.emit("error", {"message": f"Failed to remove device '{device_name}'"})

        except Exception as e:
            self.gui_event.emit("error", {"message": f"Error removing device: {str(e)}"})
    ...
    def _handle_update_device(self, device_name: str, updated_data: dict):
        ...
        try:
            ...
            if success:
                ...
                # Emit success event
                self.gui_event.emit("device_updated", {"device_name": device_name, "data": updated_data})
            else:
                self.gui_event.emit("error", {"message": f"Failed to update device '{device_name}'"})

        except Exception as e:
            self.gui_event.emit("error", {"message": f"Error updating device: {str(e)}"})
    ...
    def _handle_selection_changed(self, device_name: str):
        ...
        try:
            ...
            self.gui_event.emit("selection_changed", {"device_name": device_name})
        ...
        try:
            if hasattr(self._model, 'all_devices_model'):
                ...
                for row in range(self._model.all_devices_model.rowCount()):
                    index = self._model.all_devices_model.index(row, 0)  # Assuming name is in column 0
    ...
    def _add_device_to_model(self, device_data: dict) -> bool:
        ...
        try:
            ...
            if hasattr(self._model, 'all_devices_model'):
                return self._model.all_devices_model.insertRow(self._model.all_devices_model.rowCount())
            return False
        except:
            return False
        ...
        try:
            if hasattr(self._model, 'all_devices_model'):
                ...
                for row in range(self._model.all_devices_model.rowCount()):
                    ...
                    if self._model.all_devices_model.data(index) == device_name:
                        return self._model.all_devices_model.removeRow(row)
            return False
        except:
            return False

    def _update_device_in_model(self, device_name: str, updated_data: dict) -> bool:
    ...
    def _get_device_data(self, device_name: str) -> typing.Optional[dict]:
        ...
        try:
            ...
            # Implementation would depend on model structure
            return {"name": device_name, "type": "Unknown", "function": "Unknown", "status": "Active"}
        except:
            return None
    ...
    def _refresh_view_data(self):
        ...
        try:
            # Refresh table views
            if hasattr(self._view, 'table_view') and self._view.table_view.model():
        ...
        try:
            ...
            if device_data and hasattr(self._model, 'selected_devices_model'):
                ...
                if success:
                    ...
                    self.gui_event.emit("device_moved_to_selected", {"device_name": device_name})

        except Exception as e:
            self.gui_event.emit("error", {"message": f"Error moving device: {str(e)}"})
    ...
    def move_device_to_available(self, device_name: str):
        ...
        try:
            ...
            if hasattr(self._model, 'selected_devices_model'):
                for row in range(self._model.selected_devices_model.rowCount()):
                    index = self._model.selected_devices_model.index(row, 0)
                    if self._model.selected_devices_model.data(index) == device_name:
                        success = self._model.selected_devices_model.removeRow(row)

                        if success:
                            self.gui_event.emit("device_moved_to_available", {"device_name": device_name})
                        break
