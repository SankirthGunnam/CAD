...
class VisualCircuitModel(QAbstractItemModel):
    ...
    def data(self, index: QModelIndex, role: int = Qt.DisplayRole) -> Any:
        ...
        if role == Qt.DisplayRole or role == ChipRoles.NameRole:
            ...
            return f"{chip_data.get('type',
                                    'Generic')} - {chip_data.get('name',
                                                                 'Unknown')}"
    ...
    def add_chip(self, chip_spec: Dict[str, Any],
    ...
    def set_chip_selection(self, selected_ids: List[str]):
        ...
        # Validate IDs
        valid_ids = [
            chip_id for chip_id in selected_ids if chip_id in self._chips]
    ...
    def _generate_pins(
            self, chip_spec: Dict[str, Any]) -> List[Dict[str, Any]]:
