...
from apps.RBM5.BCF.gui.source.visual_bcf.package_image_generator import PackageImageGenerator
...
class ChipSelectionDialog(QDialog):
    ...
    def _initialize_chip_database(self) -> Dict[str, List[Dict]]:
        ...
        return {"QORVO": [{"name": "QM77XXX Series",
                           "part_number": "QM77001",
    ...
    def _setup_ui(self):
        ...
        self.search_field.setPlaceholderText(
            "Search by name, part number, or application...")
        ...
        # Set fixed size to prevent scaling issues
        self.chip_image.setMaximumSize(250, 180)
        # Allow scaling to fill the area
        self.chip_image.setScaledContents(True)
    ...
    def _connect_signals(self):
        ...
        self.vendor_list.currentItemChanged.connect(
            self._on_vendor_selection_changed)
        self.chip_list.currentItemChanged.connect(
            self._on_chip_selection_changed)
    ...
    def _chip_matches_filters(self, chip: Dict) -> bool:
        ...
        if self.current_search:
            ...
            if not any(search_lower in field.lower()
    ...
    def _update_chip_details(self, chip_data: Dict):
        ...
        if image_name:
            pixmap = self.package_generator.get_or_create_package_image(
                image_name)
            if pixmap:
                # Scale image to fit the label with fixed size to prevent
                # continuous scaling
        ...
        if chip_data.get('description'):
            ...
            desc_value.setStyleSheet(
                "QLabel { padding: 5px; background-color: #f0f0f0; border-radius: 4px; }")
