...
class IOConnectView(QWidget):
    ...
    connection_update_requested = Signal(str, dict)  # connection_id, updated_data
    ...
    def _setup_connections_table(self, parent_layout):
        ...
        table_label.setStyleSheet("font-weight: bold; color: #34495e; font-size: 14px;")
    ...
    def _on_selection_changed(self, index):
        ...
        if index.isValid():
            ...
            if model:
                # Get connection ID from first column (assuming ID is in first column)
                connection_id = model.data(model.index(index.row(), 0), Qt.DisplayRole)
                if connection_id:
    ...
    def _on_update_clicked(self):
        ...
        if self._selected_connection_id:
            ...
            self.connection_update_requested.emit(self._selected_connection_id, updated_data)
    ...
    def _on_delete_clicked(self):
        ...
        if self._selected_connection_id:
            ...
            if reply == QMessageBox.Yes:
                self.connection_remove_requested.emit(self._selected_connection_id)
                self._clear_selection()
