...
class DeviceSettingsView(QWidget):
    ...
    def _setup_all_devices_table(self, parent_layout):
        ...
        all_devices_label.setStyleSheet(
    ...
    def _setup_selected_devices_table(self, parent_layout):
        ...
        selected_devices_label.setStyleSheet(
            "font-weight: bold; color: #34495e; font-size: 14px;")
        ...
        self.selected_devices_table.setSelectionMode(
    ...
    def _connect_signals(self):
        ...
        self.move_to_available_button.clicked.connect(
            self._on_move_to_available)
        self.refresh_button.clicked.connect(self._on_refresh_clicked)

        # Table selection signals
        if self.all_devices_table:
            self.all_devices_table.clicked.connect(
                self._on_all_devices_selection_changed)

        if self.selected_devices_table:
            self.selected_devices_table.clicked.connect(
                self._on_selected_devices_selection_changed)
    ...
    def _on_all_devices_selection_changed(self, index):
        ...
        if index.isValid():
            ...
            if model:
                ...
                device_name = model.data(model.index(
