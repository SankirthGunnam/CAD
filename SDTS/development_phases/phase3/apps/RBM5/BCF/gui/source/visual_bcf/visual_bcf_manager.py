...
class VisualBCFManager(QMainWindow):
    ...
    def __init__(self, parent=None, parent_controller=None, rdb_manager=None):
        ...
        self.setWindowTitle(
            "Visual BCF Manager - Phase 3 (Controller-centric MVC)")
    ...
    def _setup_status_bar(self):
        ...
        self.status_label = QLabel(
            "Ready - Click 'Add Component' then click on scene")
        self.status_label.setStyleSheet(
            "padding: 5px; background: #ecf0f1; border-top: 1px solid #bdc3c7;")
    ...
    def _connect_tab_controller_signals(self):
        ...
        if self.io_connect_controller:
            ...
            if hasattr(self.io_connect_controller, 'gui_event'):
                self.io_connect_controller.gui_event.connect(
                    self._on_controller_gui_event)
            # Try to connect legacy signals if available
            elif hasattr(self.io_connect_controller, 'connection_selected'):
                self.io_connect_controller.connection_selected.connect(
                    self._on_connection_selected)
                self.io_connect_controller.connection_modified.connect(
                    self._on_connection_modified)
    ...
    def _on_wire_added(
        ...
        self.status_updated.emit(
            f"Wire connected: {start_component}.{start_pin} → {end_component}.{end_pin} - Total wires: {wire_count}")
    ...
    def _on_controller_gui_event(self, event_name: str, event_data: dict):
        ...
        # Handle other events
            ...
            self.status_updated.emit(
                f"Controller event: {event_name} from {controller}")
        elif event_name == "auto_connect_completed":
            connections_created = event_data.get('connections_created', 0)
            self.status_updated.emit(
                f"Auto-connect completed: {connections_created} connections created")
