...
def test_wire_movement():
    ...
    print(
        f"✅ Created 2 components: {len(bcf_manager.scene.components)} components")
    ...
    # Force update of wires (this would normally happen automatically via
    # itemChange)
    ...
    start_moved = abs(
        initial_start_pos.x() -
        new_start_pos.x()) > 30  # Should move significantly
    start_y_moved = abs(initial_start_pos.y() -
                        new_start_pos.y()) > 30  # Y should also move
    end_unchanged = abs(initial_end_pos.x() -
