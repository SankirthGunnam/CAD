...
def test_scene_serialization():
    ...
    try:
        ...
        chip1_id = controller.add_component("TestChip1", "chip", (100, 100), {"function_type": "generic"})
        chip2_id = controller.add_component("TestChip2", "chip", (300, 100), {"function_type": "generic"})
        ...
        if chip1_id and chip2_id:
            conn_id = controller.add_connection(chip1_id, "OUT1", chip2_id, "IN1")
        ...
        print(f"   📊 Serialized components: {len(scene_data.get('components', []))}")
        print(f"   📊 Serialized connections: {len(scene_data.get('connections', []))}")
        ...
        if save_success:
            ...
            print(f"   📄 File contains {len(saved_data.get('components', []))} components")
        ...
        print(f"   🧹 After clear - Components: {len(empty_data.get('components', []))}")
        ...
        if save_success:
            ...
            if load_success:
                ...
                print(f"   📊 After load - Components: {len(loaded_data.get('components', []))}")
                print(f"   📊 After load - Connections: {len(loaded_data.get('connections', []))}")
        ...
        print(f"   💾 Save to database: {'✅ Success' if db_save_success else '❌ Failed'}")
        ...
        if db_save_success:
            ...
            print(f"   📂 Load from database: {'✅ Success' if db_load_success else '❌ Failed'}")
            
            if db_load_success:
                final_data = scene.serialize_scene()
                print(f"   📊 Final - Components: {len(final_data.get('components', []))}")
                print(f"   📊 Final - Connections: {len(final_data.get('connections', []))}")
        ...
        print(f"   • Controller save/load: {'✅' if save_success and load_success else '❌'}")
        print(f"   • Database save/load: {'✅' if db_save_success and db_load_success else '❌'}")
