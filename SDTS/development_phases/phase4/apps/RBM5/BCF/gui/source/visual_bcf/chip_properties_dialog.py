...
class ChipPropertiesDialog(QDialog):
    ...
    def _create_advanced_properties_tab(self):
        ...
        custom_layout.addRow(
            "Property 1:",
            self._create_custom_prop_row(
                self.custom_prop1_key,
                self.custom_prop1_value))
        custom_layout.addRow(
            "Property 2:",
            self._create_custom_prop_row(
                self.custom_prop2_key,
                self.custom_prop2_value))
        custom_layout.addRow(
            "Property 3:",
            self._create_custom_prop_row(
                self.custom_prop3_key,
                self.custom_prop3_value))
        ...
        for checkbox in [
                self.feature_bypass,
                self.feature_shutdown,
                self.feature_temp_sensor,
                self.feature_digital_ctrl,
                self.feature_esd_protection]:
        ...
        self.notes_edit.setPlaceholderText(
            "Additional notes, design considerations, or special requirements...")
    ...
    def _create_custom_prop_row(
            self,
            key_edit: QLineEdit,
            value_edit: QLineEdit) -> QWidget:
    ...
    def _populate_fields(self):
        ...
        if chip_type in [
            self.type_combo.itemText(i) for i in range(
                self.type_combo.count())]:
        ...
        self.insertion_loss_edit.setText(
            self.chip_data.get('insertion_loss', ''))
        ...
        self.supply_voltage_edit.setText(
            self.chip_data.get('supply_voltage', ''))
        self.supply_current_edit.setText(
            self.chip_data.get('supply_current', ''))
    ...
    def _update_chip_data(self):
        ...
        if self.insertion_loss_edit.text().strip():
            self.chip_data['insertion_loss'] = self.insertion_loss_edit.text(
            ).strip()
        ...
        if self.supply_voltage_edit.text().strip():
            self.chip_data['supply_voltage'] = self.supply_voltage_edit.text(
            ).strip()
        if self.supply_current_edit.text().strip():
            self.chip_data['supply_current'] = self.supply_current_edit.text(
            ).strip()
