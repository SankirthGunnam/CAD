...
from PySide6.QtWidgets import (QTreeView, QApplication, QHeaderView, QMainWindow, 
                               QMessageBox, QStyledItemDelegate, QInputDialog, QWidget)
from PySide6.QtCore import QAbstractItemModel, QModelIndex, QObject, Qt, QFileInfo, Slot, QTimer
from PySide6.QtGui import QCloseEvent


class InputDialogDelegate(QStyledItemDelegate):
    """Custom delegate that uses QInputDialog for editing values"""
    ...
    def createEditor(self, parent, option, index):
        """Create editor - we use InputDialog instead of inline editor"""
        ...
        # Create a minimal hidden widget that will be used as parent for the dialog
        editor = QWidget(parent)
        editor.hide()
        
        # Open InputDialog using QTimer to ensure it runs after editor is created
        QTimer.singleShot(10, lambda: self._open_input_dialog(index, editor))
        
        return editor
    
    def _open_input_dialog(self, index: QModelIndex, editor_widget):
        """Open InputDialog to edit the value"""
        ...
        key = index.siblingAtColumn(0).data(Qt.ItemDataRole.DisplayRole)
        
        # Create dialog title
        title = f"Edit {key}" if key else "Edit Value"
        
        # Get the main window as parent for the dialog
        parent = editor_widget.parent() if editor_widget else None
    ...
    def setEditorData(self, editor, index):
        """Set editor data - not used since we use InputDialog"""
        pass
    
    def setModelData(self, editor, model, index):
        """Set model data - handled in _open_input_dialog"""
        pass
    
    def updateEditorGeometry(self, editor, option, index):
        """Update editor geometry - not used since we use InputDialog"""
        if editor:
...
class JsonTreeViewWindow(QMainWindow):
    ...
    def __init__(self, model: JsonModel, file_path: str = None):
        ...
        # Set custom delegate for editing with InputDialog
        delegate = InputDialogDelegate(self)
